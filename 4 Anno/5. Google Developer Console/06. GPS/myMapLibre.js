class MyMapLibre {
    ...
    */
    ...
    /**
     *
     * @param longitude longitudine o indirizzo fisico
     * @param latitude latitudine, da non mettere se si inserisce l'indirizzo fisico
     * @returns restituisce il gpsAddress
     */
    async geocode(longitude, latitude) {
        ...
        let url;
        //Se latitude esiste vuol dire che gli son state passate le coordinate
        //altrimenti vuol dire che gli è stato passato solo un link
        if (latitude) {
            url = geocodingURL
                + `/${longitude},${latitude}.json?key=`
                + this.API_KEY
        }
        else {
            let indirizzoFisico = longitude
            url = geocodingURL
                + `/${encodeURIComponent(longitude)}.json?key=`
                + this.API_KEY
        }
