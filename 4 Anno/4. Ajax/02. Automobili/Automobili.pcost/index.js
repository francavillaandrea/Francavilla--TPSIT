...
async function caricaDati() {
    ...
    try {
        const resMarche = await ajax.sendRequest("GET", "marche");
        popolaSelect(lstMarche, resMarche.data, m => m.nome, "Seleziona Marca");

        const resModelli = await ajax.sendRequest("GET", "modelli");
        modelli = resModelli.data;

        const resAutomobili = await ajax.sendRequest("GET", "automobili");
        automobili = resAutomobili.data;
    } catch (err) {
        ajax.errore(err);
    }
}
...
tbody.addEventListener("click", async e => {
    ...
    if (target.classList.contains("btnElimina")) {
        ...
        try {
            await ajax.sendRequest("DELETE", "automobili/" + autoSelezionata.id);
            riga.remove();
...
btnAggiorna.addEventListener("click", async () => {
    ...
    try {
        await ajax.sendRequest("PATCH", "automobili/" + autoCorrente.id, { prezzo: nuovoPrezzo });
